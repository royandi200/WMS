...
function assertDocumentTypeMarker(expectedType, evidenceText) {
  ...
  if (Object.values(markers).filter(Boolean).length > 1) {
    throw inputError('El PDF contiene marcadores contradictorios de orden de compra, salida o recepcion 3Q');
  }
  ...
  if (expectedType === DOCUMENT_TYPES.OUTSOURCING_RECEIPT && !markers.outsourcingReceipt) {
    throw inputError('El PDF debe incluir el encabezado visible ENTREGA DE PRODUCTO TERMINADO - MAQUILA 3Q');
  }
}
