...
const { confirmProductionReplenishment } = require('../../_lib/production-replenishment');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const body = req.body || {};
    const data = await confirmProductionReplenishment({
    ...
    console.error('[production/replenishment-confirm]', error.message);
    return res.status(500).json({ ok: false, error: 'Error al confirmar la reposicion' });
  }
};
