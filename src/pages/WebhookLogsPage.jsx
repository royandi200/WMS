  ...
  return (
    <div>
      ...
      <form onSubmit={handleFilter} className="flex flex-wrap gap-3 mb-6">
        ...
        <button type="submit" className="self-end px-4 py-[9px] bg-primary hover:bg-primary-hover text-white text-sm font-semibold rounded-md transition-colors">
          Filtrar
        </button>
      </form>
        ...
        <>
                ...
                {expanded === log.id && (
                  <div className="border-t border-border/50 px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <pre className="text-xs text-foreground bg-bg rounded p-3 overflow-auto max-h-48">
          ...
          <div className="flex items-center justify-between mt-4">
            <p className="text-xs text-muted">
              Página {filters.page || 1} · {meta.total || 0} logs
            </p>
            <div className="flex gap-2">
              ...
              >← Anterior</button>
              <button
                disabled={logs.length < (filters.limit || 20) || loading}
                onClick={() => handlePage((filters.page || 1) + 1)}
                className="px-3 py-1.5 text-xs rounded border border-border text-muted hover:text-foreground hover:border-primary/50 disabled:opacity-30 transition-colors"
              >Siguiente →</button>
            </div>
