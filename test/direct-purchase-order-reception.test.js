...
test('dashboard starts physical reception from an open purchase order', () => {
  ...
  assert.match(page, /Iniciar recepción física/u);
  assert.match(page, /Pendiente de la OC/u);
  assert.match(page, /Lote proveedor \*/u);
  assert.match(page, /Vencimiento \*/u);
  assert.match(page, /Ubicación \*/u);
  assert.match(page, /Cantidad física \*/u);
  assert.match(page, /Selecciona condición/u);
  assert.match(page, /ubicación preferida.*otra ubicación activa/iu);
