...
### Correcciones D02/D08 y resumen documental - 2026-09-06, 15:15-15:26 Bogota

- Dashboard: endpoint de agregados SQL del periodo completo, sin limites de 50/100/200 filas para KPI; lectura transaccional consistente y sin escrituras. Ventanas de 1/7/30 dias calendario de Bogota. Solo la lista de actividad reciente se limita explicitamente a ocho movimientos.
- Unidades: cantidades separadas por unidad; gr/g se agrupan como g. No se suman piezas y masa. Stock disponible excluye lotes bloqueados/vencidos, posiciones invalidas e inactivas. Rechazado se obtiene de la condicion fisica registrada, no de una resta entre esperado y recibido. Entradas/salidas provienen del signo de los movimientos reales del Kardex; no incluyen reservas sin movimiento.
- UI: errores de lectura visibles, sin presentar una lectura fallida como actualizacion exitosa; cambio de periodo protegido frente a respuestas tardias. Aprobaciones no se consultan sin capacidad approvals.read.
- Fechas: webhook utiliza la misma conexion/interpretacion MySQL DATETIME de Bogota que dashboard. No se desplazaron ni reescribieron fechas almacenadas. Verificacion SQL directa del ultimo movimiento contra su representacion Bogota aprobada.
- Trazabilidad: consumo efectivo separado de reservas no consumidas. Reposiciones canceladas y OP canceladas no se muestran como reservas activas; entregas efectivamente realizadas y luego devueltas conservan su historia.
- PDF: ambas respuestas documentales resumen las lineas persistidas por unidad, incluso en reintentos; no resumen el payload OCR entrante. R09 comprobado en lectura real: 8.750 g y 376 und.
