...
Politica documental: se instruye al usuario a adjuntar en el mismo mensaje del PDF una indicacion como `Orden de compra` o `Remision de salida a 3Q`. Sin embargo, el adaptador Meta de BuilderBot usado actualmente no conserva de forma confiable `message.document.caption`: para eventos de documento entrega un marcador y el OCR, pero no permite distinguir con certeza si el PDF llevaba texto. Por eso esta regla no se anuncia como un control automatico. Todo PDF admitido solo crea un borrador revisable; nunca crea una OC operativa, confirma una salida, reserva ni modifica inventario. Un texto posterior tampoco recupera ni vincula el archivo anterior. Si BuilderBot incorpora soporte verificable para caption de documentos, la API ya dispone de `assertDocumentHasSameMessageInstruction` para activar el rechazo estricto usando un campo contractual separado del OCR.

Enrutamiento documental: la clasificacion se fundamenta en un encabezado visible dentro del PDF, no en su nombre de archivo ni en el caption. Una OC debe contener `ORDEN DE COMPRA`; una remision de materiales debe contener `SALIDA DE BODEGA HACIA 3Q` o `REMISION A 3Q`. BuilderBot clasifica el documento y la API vuelve a validar el marcador contra el OCR. Marcadores ausentes o contradictorios fallan de forma cerrada. Los borradores `ORDEN_COMPRA` se muestran en `Recepciones > Ordenes de compra`; los `SALIDA_BODEGA_3Q`, en `Maquila 3Q`.
