...
4. Datana recibe **2** Booster disponibles en C8, lote `TRIO-E-3Q-A`, vencimiento 2026-09-14. Puede hacerlo en `Recepciones > Confirmar recepcion > Producto desde 3Q` o por WhatsApp contra `MQ ID N`, incluyendo los datos en texto o en el PDF de entrega 3Q. WhatsApp debe mostrar una vista previa y exigir despues `Confirmo la recepcion MQ ID N`. Debe quedar acumulado 2 de 4, saldo pendiente 2 y orden abierta.
...
- **Recepcion 3Q por WhatsApp habilitada despues de conciliar:** la remision y la OC se vinculan exclusivamente y de forma manual en el dashboard. Una vez vinculadas, WhatsApp puede preparar, mostrar la vista previa y confirmar una entrega parcial o total contra `MQ ID N`. Los datos fisicos pueden escribirse o extraerse del PDF adjunto, pero deben llegar completos en un solo mensaje; sigue aplazada la acumulacion de datos enviados por partes. El PDF solo nunca concilia, confirma ni modifica inventario.
- **Identificadores WhatsApp sin colision:** las recepciones directas se muestran y seleccionan como `OC ID N`; las de maquila, como `MQ ID N`. Un numero solo, `ID N` o una seleccion posicional se considera ambiguo y no puede preparar ni confirmar ninguna recepcion. `MQ ID` usa las acciones exclusivas de recepcion 3Q y nunca `PREPARAR_RECEPCION_OC`.
...
6. Recibir entregas parciales o totales de 3Q desde el dashboard o WhatsApp, conservando lote, vencimiento, condicion y ubicacion por cada entrega. En WhatsApp siempre se muestra una vista previa y se exige una confirmacion final separada con `MQ ID N`.
