...
export default function WebhookLogsPage() {
  ...
  const {
    ...
    fetchLogs, setFilters, clearError,
  } = useWebhookStore()
    ...
    <div>
      ...
      <form onSubmit={handleFilter} className="flex flex-wrap gap-3 mb-6">
        <div>
          ...
          <select value={filters.status || ''} onChange={(e) => setFilters({ status: e.target.value || undefined })} className="input-field py-1.5 pr-8 text-sm">
            <option value="">Todos</option>
            <option>RECEIVED</option><option>PROCESSED</option><option>REJECTED</option><option>ERROR</option>
          </select>
        </div>
        <div>
          <label className="block text-xs text-muted mb-1">Prioridad</label>
          <select value={filters.priority || ''} onChange={(e) => setFilters({ priority: e.target.value || undefined })} className="input-field py-1.5 pr-8 text-sm">
        ...
        <div>
          ...
          <input value={filters.from_phone || ''} onChange={(e) => setFilters({ from_phone: e.target.value || undefined })} placeholder="+57300..." className="input-field py-1.5 text-sm w-44" />
        </div>
        <button type="submit" className="self-end px-4 py-[9px] bg-primary hover:bg-primary-hover text-white text-sm font-semibold rounded-md transition-colors">Filtrar</button>
      </form>
      ...
      {!loading && logs.length > 0 && (
        <>
            ...
            {logs.map((log) => (
              <div key={log.id} className="bg-surface border border-border rounded-lg overflow-hidden">
                <button onClick={() => setExpanded(expanded === log.id ? null : log.id)}
                  ...
                  <span className="text-xs text-muted whitespace-nowrap">{log.created_at?.slice(0,16).replace('T',' ')}</span>
                ...
                {expanded === log.id && (
                  <div className="border-t border-border/50 px-4 py-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <pre className="text-xs text-foreground bg-bg rounded p-3 overflow-auto max-h-48">{JSON.stringify(log.payload, null, 2)}</pre>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Respuesta enviada</p>
                      <pre className="text-xs text-foreground bg-bg rounded p-3 overflow-auto max-h-48">{JSON.stringify(log.response, null, 2)}</pre>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="flex items-center justify-between mt-4">
            <p className="text-xs text-muted">Página {filters.page || 1} · {meta.total || 0} logs</p>
            <div className="flex gap-2">
              <button disabled={(filters.page || 1) <= 1 || loading} onClick={() => handlePage((filters.page || 1) - 1)}
                className="px-3 py-1.5 text-xs rounded border border-border text-muted hover:text-foreground hover:border-primary/50 disabled:opacity-30 transition-colors">← Anterior</button>
              <button disabled={logs.length < (filters.limit || 20) || loading} onClick={() => handlePage((filters.page || 1) + 1)}
                className="px-3 py-1.5 text-xs rounded border border-border text-muted hover:text-foreground hover:border-primary/50 disabled:opacity-30 transition-colors">Siguiente →</button>
            </div>
          </div>
        </>
      )}
