            ...
            <tbody>
              ...
              {rows.map((row) => {
                ...
                return <tr key={row.id} className="border-b border-border/50 hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-3 font-mono text-xs">{row.orden_compra_numero}</td>
...
function CreateForm({ purchaseOrders, loading, onSubmit }) {
  const [form, setForm] = useState({ orden_compra_id: '', sku: '', cantidad_objetivo: '', notas: '' })
  const set = (key) => (event) => setForm((current) => ({ ...current, [key]: event.target.value }))
  const submit = (event) => {
    event.preventDefault()
    onSubmit({ ...form, orden_compra_id: Number(form.orden_compra_id), cantidad_objetivo: Number(form.cantidad_objetivo) })
  }
  return <form onSubmit={submit} className="max-w-2xl space-y-5 border-y border-border py-5">
    <div className="grid gap-4 md:grid-cols-2">
      <Field label="Orden de compra con PDF *"><select value={form.orden_compra_id} onChange={set('orden_compra_id')} className="input-field" required><option value="">Selecciona una OC</option>{purchaseOrders.map((order) => <option key={order.id} value={order.id}>{order.numero} - {order.proveedor_nombre}</option>)}</select></Field>
    ...
    <button type="submit" disabled={loading} className="btn-primary inline-flex items-center gap-2"><Plus size={15} /> Preparar orden y remision</button>
  </form>
}
