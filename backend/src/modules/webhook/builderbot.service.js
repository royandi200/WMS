/**
 * builderbot.service.js — CORREGIDO Grupo 1
 * Dispatcher principal del webhook BuilderBot → WMS
 *
 * CORRECCIONES APLICADAS:
 *  1. AprobacionSolicitud  → ApprovalQueue  (nombre real del modelo)
 *  2. result.lots[x].qty   → qty_current    (campo real de Lot)
 *  3. b.sku                → b.siigo_code   (campo real de Product)
 *  4. stock.cantidad        → stock.qty_current / qty_initial (Lot fields)
 *  5. StockModel.sum('cantidad') — confirmado correcto en Stock
 *  6. condition 'DANADO'    → 'DESTRUCCION'  (valor original del AppScript)
 *  7. resolveLotId          → busca en tabla Lot (no en Stock)
 *  8. resolveOrderId        → usa campo codigo_orden (confirmado en ProductionOrder)
 *  9. producto.unidad       → producto.unit_label   (campo real de Product)
 * 10. Notificación al operario en APROBAR/RECHAZAR añadida
 */

const receptionSvc   = require('../reception/reception.service');
const productionSvc  = require('../production/production.service');
const dispatchSvc    = require('../dispatch/dispatch.service');
const wasteSvc       = require('../waste/waste.service');
const returnsSvc     = require('../returns/returns.service');
const approvalsSvc   = require('../approvals/approvals.service');
const inventorySvc   = require('../inventory/inventory.service');
const AppError       = require('../../utils/AppError');
const { findProductBySku, findLotByLpn, buildBotUser } = require('./builderbot.helpers');
const axios          = require('axios');

// ─────────────────────────────────────────────
// Enviar mensaje de vuelta a BuilderBot
// ─────────────────────────────────────────────
exports.sendMessage = async (phone, text) => {
  if (!process.env.BUILDERBOT_SEND_URL) return;
  await axios.post(process.env.BUILDERBOT_SEND_URL, { phone, message: text }, {
    headers: { 'x-api-key': process.env.BUILDERBOT_API_KEY || '' },
    timeout: 8000
  });
};

// ─────────────────────────────────────────────
// DISPATCHER
// ─────────────────────────────────────────────
exports.dispatch = async ({ from, action, params, priority }) => {
  const usuario = await buildBotUser(from);

  switch (action) {

    // ─── 1. INGRESO_RECEPCION ───────────────────────────────────────────────
    case 'INGRESO_RECEPCION': {
      const producto = await findProductBySku(params.id_item);
      const data = {
        producto_id:  producto.id,
        qty_total:    params.cantidad,
        qty_damaged:  params.cantidad_mala || 0,
        proveedor:    params.proveedor || null
      };
      const result = await receptionSvc.receive(data, usuario);

      // FIX #2: Lot usa qty_current, no qty
      const loteBueno      = result.lots?.find(l => l.status === 'DISPONIBLE');
      const loteCuarentena = result.lots?.find(l => l.status === 'CUARENTENA');
      const msg = [
        `✅ *Recepción registrada*`,
        `Producto: ${params.id_item}`,
        `Buenos: ${loteBueno?.qty_current ?? 0}`,
        loteCuarentena ? `Novedad: ${loteCuarentena.qty_current} (CUARENTENA)` : null,
        `LPN: *${result.lots?.[0]?.lpn ?? 'N/A'}*`
      ].filter(Boolean).join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg, lots: result.lots };
    }

    // ─── 2. SOLICITAR_INICIO_PRODUCCION ────────────────────────────────────
    case 'SOLICITAR_INICIO_PRODUCCION': {
      const producto = await findProductBySku(params.id_producto_final);
      const result   = await productionSvc.start(
        { product_id: producto.id, qty_planned: params.cantidad_planificada },
        usuario
      );

      // FIX #3: Product usa siigo_code, no sku
      const picking = result.bom_required
        .map(b => `  • ${b.siigo_code}: ${b.needed} ${b.unit}`)
        .join('\n');

      const msg = [
        `🏭 *Orden creada: ${result.order.codigo_orden}*`,
        `Producto: ${params.id_producto_final}`,
        `Cantidad: ${params.cantidad_planificada}`,
        ``,
        `📋 *Lista de recolección (FIFO):*`,
        picking,
        ``,
        `Cuando tengas los materiales responde:`,
        `✅ _Confirmo materiales para ${result.order.codigo_orden}_`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg, order_code: result.order.codigo_orden };
    }

    // ─── 3. AVANCE_FASES ───────────────────────────────────────────────────
    case 'AVANCE_FASES': {
      // FIX #8: resolveOrderId usa campo codigo_orden (confirmado en modelo)
      const orderId = await resolveOrderId(params.id_orden);
      const result  = await productionSvc.advancePhase(
        { order_id: orderId, phase: params.fase_destino },
        usuario
      );
      const faseNombre = {
        F1: 'Llenado', F2: 'Sellado', F3: 'Tapado',
        F4: 'Etiquetado', F5: 'Embalaje'
      };
      const msg = [
        `📦 *Avance registrado*`,
        `Orden: ${params.id_orden}`,
        `Fase: ${params.fase_destino} — ${faseNombre[params.fase_destino] || ''}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 4. REPORTE_MERMA ──────────────────────────────────────────────────
    case 'REPORTE_MERMA': {
      const producto = await findProductBySku(params.id_item);
      const type     = params.id_orden ? 'MERMA_EN_MAQUINA' : 'MERMA_EN_ESTANTERIA';

      // FIX #7: resolveLotId ahora busca en Lot (tabla lots), no en Stock
      const result = await wasteSvc.report({
        type,
        product_id:          producto.id,
        qty:                 params.cantidad,
        lot_id:              params.id_lote  ? await resolveLotId(params.id_lote)   : undefined,
        production_order_id: params.id_orden ? await resolveOrderId(params.id_orden): undefined,
        reason:              params.motivo
      }, usuario);

      const msg = [
        `⚠️ *Merma registrada: ${result.waste_code}*`,
        `Producto: ${params.id_item}`,
        `Cantidad: ${params.cantidad}`,
        `Motivo: ${params.motivo || 'No especificado'}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg, waste_code: result.waste_code };
    }

    // ─── 5. SOLICITAR_CIERRE_PRODUCCION ────────────────────────────────────
    case 'SOLICITAR_CIERRE_PRODUCCION': {
      const orderId = await resolveOrderId(params.id_orden);

      // FIX #1: Modelo correcto es ApprovalQueue (no AprobacionSolicitud)
      const { ApprovalQueue } = require('../../models');
      const solicitud = await ApprovalQueue.create({
        codigo_solicitud: `REQ-${Date.now().toString().slice(-6)}`,
        accion:           'SOLICITAR_CIERRE_PRODUCCION',
        payload:          { order_id: orderId, qty_real: params.cantidad_real },
        solicitado_por:   usuario.id,
        estado:           'PENDIENTE'
      });

      const msg = [
        `⏳ *Solicitud enviada: ${solicitud.codigo_solicitud}*`,
        `Orden: ${params.id_orden}`,
        `Cantidad real: ${params.cantidad_real}`,
        `Esperando aprobación del Validador.`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg, request_code: solicitud.codigo_solicitud };
    }

    // ─── 6. SOLICITAR_DESPACHO ─────────────────────────────────────────────
    case 'SOLICITAR_DESPACHO': {
      const lot = await findLotByLpn(params.id_lote);

      // FIX #1: Modelo correcto es ApprovalQueue
      const { ApprovalQueue } = require('../../models');
      const solicitud = await ApprovalQueue.create({
        codigo_solicitud: `REQ-${Date.now().toString().slice(-6)}`,
        accion:           'SOLICITAR_DESPACHO',
        payload:          { lot_id: lot.id, qty: params.cantidad, customer: params.cliente_destino },
        solicitado_por:   usuario.id,
        estado:           'PENDIENTE'
      });

      const msg = [
        `⏳ *Solicitud de despacho: ${solicitud.codigo_solicitud}*`,
        `Lote: ${params.id_lote}`,
        `Cantidad: ${params.cantidad}`,
        `Cliente: ${params.cliente_destino}`,
        `Esperando aprobación.`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg, request_code: solicitud.codigo_solicitud };
    }

    // ─── 7. GESTION_DEVOLUCION ─────────────────────────────────────────────
    case 'GESTION_DEVOLUCION': {
      const producto = await findProductBySku(params.id_item);

      // FIX #6: AppScript usaba 'DESTRUCCION' (no 'DANADO')
      const condition = params.estado === 'RECUPERABLE' ? 'RECUPERABLE' : 'DESTRUCCION';

      const result = await returnsSvc.processReturn(
        { product_id: producto.id, qty: params.cantidad, customer_origin: params.cliente_origen, condition },
        usuario
      );

      const msg = [
        `🔄 *Devolución registrada*`,
        `Producto: ${params.id_item}`,
        `Cantidad: ${params.cantidad}`,
        `Estado: ${condition}`,
        `LPN: ${result.lpn}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg, lpn: result.lpn };
    }

    // ─── 8. CONSULTAR_STOCK ────────────────────────────────────────────────
    case 'CONSULTAR_STOCK_MATERIA_PRIMA':
    case 'CONSULTAR_STOCK_PRODUCTO_TERMINADO': {
      if (params.id_item) {
        const producto = await findProductBySku(params.id_item);
        const data     = await inventorySvc.productStock(producto.id);
        const disp     = data.summary?.disponible_neto ?? 0;

        // FIX #9: Product usa unit_label, no unidad
        const msg = [
          `📊 *Stock: ${params.id_item}*`,
          `Disponible: ${disp} ${producto.unit_label || 'und'}`,
          `Lotes activos: ${data.stocks?.length || 0}`
        ].join('\n');

        await exports.sendMessage(from, msg).catch(() => {});
        return { message: msg };
      }

      const summary = await inventorySvc.globalSummary();
      const lines   = summary.slice(0, 10)
        .map(p => `  • ${p.siigo_code}: ${p.stock?.disponible_neto ?? 0}`)
        .join('\n');
      const msg = `📦 *Resumen de stock (top 10):*\n${lines}`;

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 9. CONSULTAR_ESTADO_PRODUCCION ───────────────────────────────────
    case 'CONSULTAR_ESTADO_PRODUCCION': {
      const orderId = await resolveOrderId(params.id_orden);
      const orden   = await productionSvc.getOne(orderId);
      const faseNombre = {
        F0: 'Pendiente materiales', F1: 'Llenado', F2: 'Sellado',
        F3: 'Tapado', F4: 'Etiquetado', F5: 'Embalaje/Cierre'
      };

      const msg = [
        `🔍 *Orden: ${params.id_orden}*`,
        `Producto: ${orden.producto?.nombre ?? 'N/A'}`,
        `Estado: ${orden.estado}`,
        `Fase: ${orden.fase} — ${faseNombre[orden.fase] || ''}`,
        `Planificado: ${orden.cantidad_planeada}`,
        `Real: ${orden.cantidad_real || 'en proceso'}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 10. CONSULTAR_TRAZABILIDAD_LOTE ──────────────────────────────────
    case 'CONSULTAR_TRAZABILIDAD_LOTE': {
      // FIX #4: Lot usa qty_initial y qty_current (no 'cantidad')
      const lot = await inventorySvc.lotDetail(params.id_lote);

      const msg = [
        `🔎 *Trazabilidad: ${params.id_lote}*`,
        `Producto: ${lot.producto?.nombre ?? 'N/A'}`,
        `Origen: ${lot.origen ?? 'N/A'}`,
        `Estado: ${lot.estado ?? lot.status}`,
        `Cantidad inicial: ${lot.qty_initial}`,
        `Cantidad actual: ${lot.qty_current}`,
        `Proveedor: ${lot.proveedor ?? lot.supplier ?? 'N/A'}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 11. CONSULTAR_CAPACIDAD_FABRICACION ──────────────────────────────
    case 'CONSULTAR_CAPACIDAD_FABRICACION': {
      const producto = await findProductBySku(params.id_producto_final);
      const { BOM, Stock: StockModel } = require('../../models');

      const bom    = await BOM.findAll({ where: { producto_final_id: producto.id } });
      const checks = [];
      let puedeProd = true;

      for (const item of bom) {
        const necesario  = parseFloat(item.cantidad_por_unidad) * params.cantidad_deseada;
        // FIX #5: Stock.cantidad es el campo correcto — confirmado en modelo Stock
        const disponible = await StockModel.sum('cantidad', {
          where: { producto_id: item.insumo_id }
        }) || 0;
        const ok = disponible >= necesario;
        if (!ok) puedeProd = false;

        checks.push(`  ${ok ? '✅' : '❌'} ${item.insumo_id}: necesita ${necesario}, tiene ${disponible}`);
      }

      const msg = [
        `${puedeProd ? '✅' : '❌'} *Capacidad para ${params.cantidad_deseada} uds de ${params.id_producto_final}:*`,
        ...checks
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg, can_produce: puedeProd };
    }

    // ─── 12. APROBAR_SOLICITUD ─────────────────────────────────────────────
    case 'APROBAR_SOLICITUD': {
      const result = await approvalsSvc.approve(params.id_solicitud, usuario);

      // FIX #10: Notificar al operario que fue aprobado
      if (result.solicitante?.phone) {
        const msjOperario = [
          `✅ *Tu solicitud fue aprobada*`,
          `ID: ${params.id_solicitud}`,
          `Acción: ${result.accion?.replace(/_/g, ' ')}`,
          `Aprobado por: ${usuario.nombre}`
        ].join('\n');
        await exports.sendMessage(result.solicitante.phone, msjOperario).catch(() => {});
      }

      const msg = [
        `✅ *${params.id_solicitud} Aprobada*`,
        `Acción: ${result.accion?.replace(/_/g, ' ')}`,
        `Aprobado por: ${usuario.nombre}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 13. RECHAZAR_SOLICITUD ────────────────────────────────────────────
    case 'RECHAZAR_SOLICITUD': {
      const result = await approvalsSvc.reject(
        { codigo_solicitud: params.id_solicitud, motivo: params.motivo },
        usuario
      );

      // FIX #10: Notificar al operario que fue rechazado
      if (result.solicitante?.phone) {
        const msjOperario = [
          `❌ *Tu solicitud fue rechazada*`,
          `ID: ${params.id_solicitud}`,
          params.motivo ? `Motivo: ${params.motivo}` : null
        ].filter(Boolean).join('\n');
        await exports.sendMessage(result.solicitante.phone, msjOperario).catch(() => {});
      }

      const msg = [
        `❌ *${params.id_solicitud} Rechazada*`,
        params.motivo ? `Motivo: ${params.motivo}` : ''
      ].filter(Boolean).join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 14. CONFIRMAR_MATERIALES_PRODUCCION ──────────────────────────────
    case 'CONFIRMAR_MATERIALES_PRODUCCION': {
      const orderId = await resolveOrderId(params.id_orden);

      // FIX #7: resolveLotId ahora apunta a tabla Lot correctamente
      const result = await productionSvc.confirmMaterials({
        order_id:         orderId,
        exception_lot_id: params.lote_usado ? await resolveLotId(params.lote_usado) : undefined
      }, usuario);

      const msg = [
        `✅ *Materiales confirmados*`,
        `Orden: ${params.id_orden}`,
        `Fase: ${result.phase}`,
        `Lotes consumidos: ${result.consumed?.length || 0}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 15. EXCEPCION_PICKING ─────────────────────────────────────────────
    case 'EXCEPCION_PICKING': {
      const orderId = params.id_orden ? await resolveOrderId(params.id_orden) : null;

      // FIX #7: resolveLotId apunta a Lot
      const lotId = await resolveLotId(params.lote_usado);
      const result = await productionSvc.confirmMaterials(
        { order_id: orderId, exception_lot_id: lotId },
        usuario
      );

      const msg = [
        `🔄 *Excepción de picking registrada*`,
        `Orden: ${params.id_orden || 'N/A'}`,
        `Lote sugerido: ${params.lote_sugerido}`,
        `Lote usado: ${params.lote_usado}`
      ].join('\n');

      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    // ─── 16. MODO_CHARLA ───────────────────────────────────────────────────
    case 'MODO_CHARLA': {
      const msg = params.texto || 'No entendí tu mensaje. ¿Puedes ser más específico?';
      await exports.sendMessage(from, msg).catch(() => {});
      return { message: msg };
    }

    default:
      throw new AppError(`Acción desconocida: ${action}`, 400);
  }
};

// ─────────────────────────────────────────────
// Helpers internos
// ─────────────────────────────────────────────

// FIX #8: usa campo codigo_orden — confirmado en ProductionOrder.js
async function resolveOrderId(codigoOrden) {
  const { ProductionOrder } = require('../../models');
  const o = await ProductionOrder.findOne({ where: { codigo_orden: codigoOrden } });
  if (!o) throw new AppError(`Orden ${codigoOrden} no encontrada`, 404);
  return o.id;
}

// FIX #7: busca en tabla 'lots' (modelo Lot) por campo 'lpn' — NO en Stock
async function resolveLotId(lpn) {
  const { Lot } = require('../../models');
  if (!Lot) throw new AppError('Modelo Lot no disponible. Verifica models/index.js', 500);
  const lot = await Lot.findOne({ where: { lpn } });
  if (!lot) throw new AppError(`Lote ${lpn} no encontrado`, 404);
  return lot.id;
}
