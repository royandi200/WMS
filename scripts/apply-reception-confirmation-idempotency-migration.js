...
const MIGRATION_PATH = path.join(__dirname, '..', 'database', '21_reception_confirmation_idempotency.sql');
  ...
  const [columns] = await conn.execute(
    `SELECT COLUMN_TYPE
      ...
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'recepciones'
        AND COLUMN_NAME = 'confirmacion_clave'`
  );
  const [indexes] = await conn.execute(
    `SELECT INDEX_NAME
       FROM information_schema.STATISTICS
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'recepciones'
        AND INDEX_NAME = 'uk_recepcion_confirmacion_clave'`
  );
  return {
    confirmationColumn: columns.length === 1,
    confirmationIndex: indexes.length === 1,
  };
}
...
async function main() {
  ...
  try {
    ...
    if (before.confirmationColumn && before.confirmationIndex) {
      console.log(JSON.stringify({ ok: true, mode: 'already-applied', before }, null, 2));
      return;
    }
    if (before.confirmationColumn || before.confirmationIndex) {
      throw new Error('La migracion 21 esta aplicada parcialmente');
    }
    if (!apply) {
    ...
    if (!after.confirmationColumn || !after.confirmationIndex) {
      throw new Error('La migracion 21 no quedo completa');
    }
