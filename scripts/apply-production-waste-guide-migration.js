...
async function main() {
  ...
  const source = fs.readFileSync(path.join(__dirname,
    '../database/37_production_waste_guided.sql'), 'utf8');
  const conn = await createConnection();
  try {
    const [rows] = await conn.execute(
      `SELECT COUNT(*) AS present FROM information_schema.TABLES
        WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'produccion_merma_borradores'`
    );
    ...
    process.stdout.write(`${JSON.stringify({ mode: apply ? 'apply' : 'dry-run',
      table: 'produccion_merma_borradores', presentBefore: present,
