...
const MIGRATION_PATH = path.join(__dirname, '..', 'database', '32_user_whatsapp_aliases.sql');
...
async function inspect(conn) {
  ...
  return { aliasTable: tables.length === 1 };
}
...
async function main() {
  ...
  try {
    ...
    if (!after.aliasTable) throw new Error('La migracion no quedo aplicada');
