...
const MIGRATION_PATH = path.join(__dirname, '..', 'database', '34_builderbot_durable_inbox.sql');
...
async function inspect(conn) {
  const [tables] = await conn.execute(
    `SELECT TABLE_NAME FROM information_schema.TABLES
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'webhook_ingress_inbox'`
  );
  return { durableInboxTable: tables.length === 1 };
}
...
async function main() {
  ...
  try {
    ...
    if (!after.durableInboxTable) throw new Error('La migración de bandeja durable no quedó aplicada');
