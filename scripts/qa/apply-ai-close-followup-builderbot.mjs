...
if (!key || backup.prompts.length !== 2) throw new Error('Missing Builderbot key or rollback checkpoint');
...
function section(source) {
  ...
  if (start < 0 || end < 0 || source.indexOf('### 10.', start + 1) >= 0) {
    throw new Error('Cannot isolate section 10 of Builderbot prompt');
  }
  return source.slice(start, end);
}

function patch(original) {
  const prior = section(original);
  const desired = section(local);
  if (unix(prior) === unix(desired)) return original;
  if (unix(prior) !== unix(section(baseline))) {
    throw new Error('Live section 10 diverged from the known baseline; no overwrite');
  }
  const newline = original.includes('\r\n') ? '\r\n' : '\n';
  return original.replace(prior, unix(desired).replace(/\n/gu, newline));
}
...
const changes = backup.prompts.map(saved => {
  const before = instructions(live, saved);
  const expected = restore ? applied.prompts.find(item => item.name === saved.name)?.instructions
    : saved.instructions;
  if (before !== expected && !(restore && before === saved.instructions)) {
    throw new Error(`Live ${saved.name} differs from checkpoint; no overwrite`);
  }
  return { saved, before, after: restore ? saved.instructions : patch(before) };
});
if (!apply) {
...
async function mcpSession(operation) {
  ...
  try {
    await send({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {
      ...
      clientInfo: { name: 'wms-ai-close-followup-sync', version: '1.0.0' },
    } });
