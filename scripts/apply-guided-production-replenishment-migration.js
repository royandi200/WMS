...
async function main() {
  ...
  const source = fs.readFileSync(path.join(__dirname,
    '../database/35_production_replenishment_guided.sql'), 'utf8');
  ...
  try {
    const [rows] = await conn.execute(
      `SELECT COUNT(*) AS present
        ...
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'produccion_reposicion_borradores'`
    );
    ...
    process.stdout.write(`${JSON.stringify({ mode: apply ? 'apply' : 'dry-run',
      table: 'produccion_reposicion_borradores', presentBefore: present,
