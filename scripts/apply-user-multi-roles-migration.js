...
const MIGRATION_PATH = path.join(__dirname, '..', 'database', '36_user_multi_roles_and_outbound_aliases.sql');
...
async function inspect(conn) {
  ...
  const [columns] = await conn.execute(
    `SELECT COLUMN_NAME FROM information_schema.COLUMNS
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'usuario_whatsapp_aliases'
        AND COLUMN_NAME = 'habilitado_salida'`
  );
  return { multiRoleTable: tables.length === 1, outboundAliasColumn: columns.length === 1 };
}
...
async function main() {
  ...
  try {
    ...
    if (before.multiRoleTable && before.outboundAliasColumn) {
    ...
    for (const statement of statements) {
      if (before.outboundAliasColumn && /ADD COLUMN habilitado_salida/iu.test(statement)) continue;
      await conn.query(statement);
    }
    const after = await inspect(conn);
    if (!after.multiRoleTable || !after.outboundAliasColumn) {
      throw new Error('La migracion de roles multiples y aliases salientes quedo incompleta');
    }
